...
typedef struct SAMPLING_SCHEME {
	...
	int sizeParamTime, sizeParamBirth, sizeParamDeath, sizeParamFossil, sizeParamSampling, *schemeTime, *schemeBirth, *schemeDeath, *schemeFossil, *schemeSampling;
} TypeSamplingScheme;


typedef struct SAMPLING_CURRENT {
	double *currentBirth, *currentDeath, *currentFossil, *currentSampling;
} TypeSamplingCurrent;
