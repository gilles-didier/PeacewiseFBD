...
int main(int argc, char **argv) {	
	...
	FILE *fi;
	int i, j, debug = 0;
	double minTimeIntInf = NO_TIME, maxTimeIntSup = 0., maxDisplayTime = 0., figwidth = 100.;
	...
	if((fi = fopen(inputFileNameTree, "r"))) {
		...
		fixStatus(tree, fos);
		for(n=0; n<tree->size; n++)
			if(tree->node[n].child == NOSUCH && fos->status[n] == unknownNodeStatus)
				tree->time[n] = fos->endTimeTable[n].inf;
