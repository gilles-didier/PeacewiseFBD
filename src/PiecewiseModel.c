...
char setSamplingSchemeTime(FILE *f, int *size, int *scheme, double *value) {
	int i;
	char c;
	char tmp[MAX_NAME_SIZE+1];
	//for(c = fgetc(f); c != EOF && issepline(c); c = fgetc(f))
...
TypeSamplingScheme readSamplingScheme(FILE *f) {
    ...
    do {
		c = setSamplingSchemeTime(f, &(sampScheme.sizeParamTime), &(sampScheme.schemeTime[sampScheme.param.size]), &(sampScheme.param.startTime[sampScheme.param.size]));
		if(c!=EOF) {
