...
static double getLogDensitySum(TypeTree **tree, int nTree, TypeFossilFeature *fp, TypePiecewiseModelParam *param);
...
TypeTypeVariableData getTypeVariableData(double probSpe, double probExt, double probFos, TypeSamplingScheme sampScheme) {
	TypeTypeVariableData tvd;
	double totProb = 0., probSampling = 1.-probSpe-probExt-probFos;
	int i;
...
double MCMCGetMaxLogLikelihoodSCNLOpt(TypeTree **tree, int nTree, TypeFossilIntFeature *fi, TypeSamplingScheme sampScheme, TypeNLOptOption *option, double al, int iter, double prop, TypeModelParam *windSize, TypeModelParam *init, double probSpe, double probExt, double probFos, void *rand_data) {
	int *branch, i, s, n;
	double prob, max = 0.;
	TypeExtendedFossilFeature *eff;
	...
	tvd = getTypeVariableData(probSpe, probExt, probFos, sampScheme);
...
//						error("Node %d (%s) has unknown status\n", n, tree[i]->name[n]);
	...
	max = prob;
	estim.param = getPiecewiseParamFromSamplingScheme(sampScheme);
	for(s=0; s<iter/2; s++) {
...
if(s%10 == 0) {
	...
	for(s=0; s<iter; s++) {
		...
		if(prob>max)
			max = prob;
if(s%10 == 0) {
	fprintf(stderr, "it %-7d/%d %le %le\n", s, iter, max, prob); //fflush(stderr);
}
if(s%50 == 0) {
	minimizePiecewiseParamFromSetTreeFossil(getLogDensitySum, tree, nTree, eff->ff, sampCurrent, sampScheme, option, &estim);
	fprintf(stdout, "Result %le\t%le\n", max, estim.logLikelihood);
}
	}
	estim.param = getPiecewiseParamFromSamplingScheme(sampScheme);
	minimizePiecewiseParamFromSetTreeFossil(getLogDensitySum, tree, nTree, eff->ff, sampCurrent, sampScheme, option, &estim);
	...
	freePiecewiseParam(&(estim.param));
	return max;
}
...
/*it is assumed that all the branch with identifiers in the trees have the same index, the tree nodes with fossils must have names*/
double MCMCFillMaxParamSampleSC(TypeTree **tree, int nTree, TypeFossilIntFeature *fi, TypeSamplingScheme sampScheme, double al, int iter, double prop, TypeModelParam *windSize, TypeModelParam *init, double probSpe, double probExt, double probFos, TypePiecewiseModelParam *maxParam, void *rand_data) {
	int *branch, i, s, n;
	double prob, max = 0.;
	TypeExtendedFossilFeature *eff;
	TypePiecewiseModelParam param;
