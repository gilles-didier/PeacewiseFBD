...
int main(int argc, char **argv) {	
	...
	if((fit = fopen(inputFileNameTree, "r")) && (fif = fopen(inputFileNameFossil, "r")) && (fis = fopen(inputFileNameScheme, "r"))) {
		...
		aic = (double*) malloc(sizeTree*sizeof(double));
		#pragma omp parallel
		{
			int i;
			#pragma omp for
			for(i=0; i<sizeTree; i++) {
				logLike = MCMCGetMaxLogLikelihoodSC(&(tree[i]), 1, fint, scheme, al, nSamp, propParam, &windSize, &init, probSpe,probExt, probFos, rand_data);
				#pragma omp critical
				{
					aic[i] = 2.*(((double)nParam)-logLike);
				}
			}
		}
		sprintf(outputFileName, "%s.csv", outputPrefix);
